...
export default function DevicesPage() {
  ...
  interface HardwareItem {
    ...
    serialNumber: string;
    description: string;
    type?: string; // e.g., 'computer', 'printer', 'audio_video', 'video', 'device'
  }
  const [deviceItems, setDeviceItems] = useState<HardwareItem[]>([]);
  ...
  const fetchDeviceItems = async () => {
    try {
      ...
      // Filter for items specifically identified as 'device'
      const dItems = data.filter(
        (item: HardwareItem) => item.type === "device"
      );
      setDeviceItems(dItems);
    } catch (err: any) {
  ...
  const handleAddOrUpdateDeviceItem = async (formData: {
    name: string;
    model: string;
    serialNumber: string;
    description: string;
    ...
    try {
      ...
      fetchDeviceItems(); // Re-fetch to update the list
  ...
  const handleDeleteDeviceItem = async (id: number) => {
    try {
  ...
  return (
    <div className="max-w-4xl mx-auto mt-8">
            ...
            <tr key={item.id}>
              ...
              <td className="py-2 px-4 border-b">
                ...
                <button
                  ...
                  onClick={() => handleDeleteDeviceItem(Number(item.id))}
                >
                  Delete
                </button>
              </td>
      ...
      <HardwareModal
        isOpen={isModalOpen}
        onClose={closeModal}
        onSubmit={handleAddOrUpdateDeviceItem}
